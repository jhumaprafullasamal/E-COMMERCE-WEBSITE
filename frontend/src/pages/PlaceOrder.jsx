...
const PlaceOrder = () => {
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t '>
      ...
      {/* ___________-RIGHT SIDE_____________ */}
      <div className="mt-8">
        ...
        <div className="mt-12">
          ...
          <div className="flex gap-3 flex-col lg:flex-row">
            <div onClick={() => setMethod('stripe')} className="flex item-center gap-3 border p-2 px-3 cursor-pointer border-zinc-200 rounded-full">
              <p className={`min-w-3.5 h-3.5 border rounded-full border-zinc-100  ${method === 'stripe' ? 'bg-green-400' : ''}`}></p>
              <img className="h-5 mx-4" src={assets.stripe_logo} alt="" />
            </div>
            <div onClick={() => setMethod('razorpay')} className="flex item-center gap-3 border p-2 px-3 cursor-pointer border-zinc-200 rounded-full">
              <p className={`min-w-3.5 h-3.5 border rounded-full border-zinc-100 ${method === 'razorpay' ? 'bg-green-400' : ''}`}></p>
              <img className="h-5 mx-4" src={assets.razorpay_logo} alt="" />
            </div>
            <div onClick={() => setMethod('cod')} className="flex item-center gap-3 border p-2 px-3 cursor-pointer border-zinc-200 rounded-full">
              <p className={`min-w-3.5 h-3.5 border rounded-full border-zinc-100 ${method === 'cod' ? 'bg-green-400' : ''}`}></p>
